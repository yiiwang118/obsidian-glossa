...
- **2026-06-30 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 🌐 Web research pipeline。** 现在可以用免费 Auto provider，或 Brave、Tavily、Exa、SerpAPI 搜索公开网页，再抓取、清洗、提取有来源的内容，而不是把整页原始内容塞给模型。
- **2026-06-30 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 📥 安全下载。** 公开 PDF、图片、release 资产、数据集等可以保存到 vault，带大小上限、私网跳转拦截、SHA-256 校验和可选 `.source.json` 来源记录。
- **2026-06-30 — ✨ 版本更新提示。** 侧栏可以提示新的 GitHub Release，也可以手动检查或忽略当前版本。
- **2026-06-29 — 🧭 长对话导航。** 左侧 conversation rail 只记录用户提问，支持当前问题高亮和快速回跳，不再把 transcript 搞得很乱。
- **2026-06-26 — 📎 上下文更清楚。** 上传文件会显示在发送后的用户消息里，输入框则把用户上传附件和当前打开文件分开处理。

<details>
<summary>从第一次社区发布准备到现在的早期更新（2026-06-23 — 2026-06-26）</summary>

- **2026-06-26 — 0.5.0 UI refresh。** 侧栏、活动行、状态 pill、PDF 上下文处理整体做了 AI Studio 风格刷新，修复重复 Thinking 和历史/session 命名问题。
- **2026-06-25 — 0.4.3 发布润色。** 改进 PDF/图片工作流提示，支持 `xhigh` 推理强度，修复选区误捕获和 custom API 工具调用问题。
- **2026-06-23 — 0.4.2 提交后修复。** 清理 Obsidian 发布检查问题，用可信 SVG 渲染替代不安全 `innerHTML`，并加入 GitHub Actions artifact provenance。
- **2026-06-23 — 0.4.1 首次社区提交准备。** 加入 `read_pdf`、release check、本地 CLI 警告、保守默认权限和更安全的大 diff 预览。
...
| 🌐 网页 / 搜索 | 抓取指定网页、搜索当前资料，或在明确 approval 后下载公开文件。 |
