...
- **2026-07-25 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 先浏览工作区，再把修改作为一次可靠事务完成。** [`0.7.3`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.7.3) 实现了 [Issue #1](https://github.com/yiiwang118/obsidian-glossa/issues/1) 请求的目录发现：Agent 可以在用户指定的工作区目录中列出有边界、可过滤的 Vault 相对路径，再直接交给笔记、PDF 与编辑工具。同一轮对一个文件的多次修改会合并为一次 diff、一次审批和一次写入；多文件补丁会先在内存中生成并核对全部结果和文件指纹，任何写入失败都会回滚。统一编辑引擎保留 BOM、换行风格、末尾换行和 Unicode，模糊匹配只作为明确显示的高置信最后手段。裁剪后的原生 PDF、供应商真实 token 用量、上下文占用明细、少量出链信息和无竞态的截图粘贴也减少了额外调用。
- **2026-07-22 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 翻译窗口会留在手边，公式也能正确排版。** [`0.7.2`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.7.2) 在原文页面滚动时保留快速翻译窗口；选区滚出视野后，窗口会停在可见区域顶部；点击左右侧栏、Glossa 侧栏、功能区与菜单也不会误关窗口。明确但被 PDF 文本层压平的公式会重建为兼容 Obsidian 的 LaTeX，并在流式输出结束后只渲染一次；普通文本继续走快速纯文本路径，缺少公式分隔符时也最多只补一次针对性纠正。
- **2026-07-22 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 翻译现在可以自然地出现在选区旁。** [`0.7.1`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.7.1) 提供三档明确触发方式：完全关闭选区触发、选区稳定后显示清晰的 Glossa 按钮，或让每个新的 PDF、Markdown 与 HTML 选区稳定 350 毫秒后自动翻译。翻译窗口顶部可以直接开关自动模式；URL、纯公式与纯代码会在请求前过滤；侧栏流式输出不再误关翻译窗口；拖入侧栏的图片也不会再落到后面的 PDF 页面。发布来源验证同时安全恢复：`main.js` 与 `styles.css` 分别生成独立 attestation，并在上传前逐一验证。
- **2026-07-20 — ![NEW](https://img.shields.io/badge/NEW-EF4444?style=flat-square) 快速翻译终于能容纳长内容，也不会丢掉原文位置。** [`0.7.0`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.7.0) 会让流式翻译窗口先随内容自然增长，达到可读上限后再出现滚动；窗口可以在可见工作区内拖动和缩放。打开窗口后，PDF 原生紫色选区会继续保留；高倍率下的文本层边界漂移也会保守校正，避免视觉上选中 `training`，实际却送出 `e trainin`。兼容版本重新降到应用 1.12.0，1.13 及以上仍会自动接入设置搜索。
...
<details>
<summary><strong>更早的更新</strong>（2026-04-08 至 2026-07-12，共 21 条）</summary>
