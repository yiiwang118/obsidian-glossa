...
- **2026-07-05 — 🛡️ 动态插件边界变得明确并能通过审查。** [`0.6.6`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.6.6) 收窄 Dataview、Tasks、Templater 和 PDF.js 集成类型，增加运行时能力判断，并消除源码审查发现的最后几处宽泛顶层类型。它保留宿主边界兼容性，同时让失败可预测，而不是藏在未经检查的值后面。
- **2026-07-04 — ✅ 社区审查会失败的问题，现在会先在本地失败。** [`0.6.5`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.6.5) 移除被禁止的 `no-explicit-any` 禁用指令，收紧并说明 lint directive，统一非 `Error` Promise rejection，并扩展发布检查以拦截目录审查同类问题。[`0.6.4`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.6.4) 作为被替代的第一次清理保留下来，审计过程仍然完整可查。
- **2026-07-04 — 🔄 更新入口回到软件内的插件页面。** [`0.6.3`](https://github.com/yiiwang118/obsidian-glossa/releases/tag/0.6.3) 将更新提示的主要操作改为打开软件内的 Glossa 插件详情页；当社区目录尚未同步时，GitHub Release 仍作为备用入口。

<details>
<summary><strong>更早的更新</strong>（2026-04-08 至 2026-07-04，共 14 条）</summary>
